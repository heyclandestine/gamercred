...
if not DATABASE_URL:
    raise ValueError("No database URL found. Set LOCAL_DATABASE_URL environment variable")
...
async def create_new_period(storage, leaderboard_type):
    ...
    try:
        ...
        if leaderboard_type == LeaderboardType.WEEKLY:
            ...
            naive_end = (naive_start + timedelta(days=7)).replace(
                hour=0, minute=0, second=0, microsecond=0
            )
            # Then localize to CST
            start_time = cst.localize(naive_start)
            end_time = cst.localize(naive_end)
        else:  # MONTHLY
            # Start from 1st of current month CST
            naive_start = naive_now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            # Create naive end time
            if naive_now.month == 12:
                naive_end = naive_now.replace(year=naive_now.year + 1, month=1, day=1,
                                     hour=0, minute=0, second=0, microsecond=0)
            else:
                naive_end = naive_now.replace(month=naive_now.month + 1, day=1,
                                     hour=0, minute=0, second=0, microsecond=0)
            # Then localize to CST
            start_time = cst.localize(naive_start)
            end_time = cst.localize(naive_end)
        ...
        print(f"Created new {leaderboard_type.value} period: {start_time} to {end_time}")
        return new_period
    finally:
        session.close()
